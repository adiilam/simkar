            ...
            <div class="card-header">
                <div class="row">
                    <?php if ($this->utils->isPermAllowed($_SESSION['perm']->karyawanCreate ?? '')) : ?>
                        <div class="col-auto">
                            <button class="btn btn-primary" data-toggle="modal" data-target="#modaltambahBank">
                        ...
                        <tbody>
                            ...
                            ?>
                                <thead class="text-center">
                                    <tr>
                                        <td><?php echo $no++ ?></td>
                                        <td><?php echo $bnk->nama ?></td>
                                        <td><?php echo $bnk->bank ?></td>
                                        <td><?php echo $bnk->no_rekening ?></td>
                                        <td><?php echo $bnk->cabang ?></td>
                                        <td>
                                            <?php if ($this->utils->isPermAllowed($_SESSION['perm']->karyawanUpdate ?? '')) : ?>
                                                <button class="btn btn-primary btn-sm" title="Edit" onclick="openEditBank('<?= $bnk->uid ?>')">
                                                    <i class="fa fa-edit"></i>
                                                </button>
                                            <?php endif; ?>
                                            <?php if ($this->utils->isPermAllowed($_SESSION['perm']->karyawanDelete ?? '')) : ?>
                                                <a class="btn btn-danger btn-sm" title="Hapus" href="<?= base_url() . "bank/delete/" . $bnk->uid . "/" . $bnk->karyawan_uid ?>" onclick="return confirm('Anda Yakin Ingin hapus?')">
...
<div class="modal fade" id="modaltambahBank" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="exampleModalLabel">Form Input Data Bank</h4>
            ...
            <div class="modal-body">
                ...
                <?php echo form_open_multipart('bank/add'); ?>
...
<div class="modal fade" id="modalEditBank" tabindex="-1" role="dialog" aria-labelledby="modalEditLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="modalEditBank">Edit Bank</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form action="<?= base_url("bank/edit") ?>" method="POST">
                    <input type="hidden" name="karyawan_uid" value="<?= $detail->uid; ?>">
                    <input type="hidden" name="uid" class="form-control" id="editUidbank" readonly>
                    <div class="form-group">
                        <label>Nama</label>
                        <input type="text" name="nama" class="form-control" id="editnamabank" required>
                    </div>
                    <div class="form-group">
                        <label>Bank</label>
                        <input type="text" name="bank" class="form-control" id="editbank">
                    </div>
                    <div class="form-group">
                        <label>No Rekening</label>
                        <input type="text" name="no_rekening" class="form-control" id="editno_rekening">
                    </div>
                    <div class="form-group">
                        <label>Cabang</label>
                        <input type="text" name="cabang" class="form-control" id="editcabang">
                    </div>
...
<script>
    ...
    function openEditBank(uid) {
        var bank = <?php echo json_encode($bank); ?>;
        var find = bank.find((value) => value.uid == uid);
        document.getElementById("editUidbank").value = find.uid;
        document.getElementById("editnamabank").value = find.nama;
        document.getElementById("editbank").value = find.bank;
        document.getElementById("editno_rekening").value = find.no_rekening;
        document.getElementById("editcabang").value = find.cabang;
